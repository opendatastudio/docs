...
In this tutorial, we'll create a dataflow that takes a table containing two
columns of time series data, and averages them along the time axis.

## Creating resources

To use tabular data in a dataflow, we need to define a **resource** to contain
it.

First, reset your dataflow to remove any previous runs and return it to a clean
state.
...
root of the dataflow. Create the `data` directory, open `data/tabulardata.csv`
