...
Then, create files for your new input `data` and output `mean` resources under the `helloworld` algorithm folder:

```bash
opends reset
mkdir helloworld/resources
touch helloworld/resources/data.json
touch helloworld/resources/mean.json
```
...
Now define the output `mean` resource:

```json title="helloworld/resources/mean.json"
{
  "name": "mean",
  "title": "Mean",
  "description": "The mean of the Y values in the input data",
  "profile": "tabular-data-resource",
  "schema": {
    "primaryKey": "time",
    "fields": [
...
This tells our algorithm that the input `data` should be a tabular data resource that conforms to the schema we defined in `helloworld/resources/data.json`. In other words, we expect a table with four columns named `time`, `y1`, `y2` and `y3`. Similarly, the output should be a table with two columns: `time` and `mean`.

## Modifying the algorithm

Finally, we can modify our algorithm to take the input data and use it to calculate the mean Y values. Note that by default, tabular data resources are passed to the algorithm as named pandas DataFrames.
...
Here we've averaged the `y1`, `y2` and `y3` columns together to get the values in `mean`.

Next, we'll learn about working with multiple runs in a datapackage.
