...
description: Working with tabular data resources in a dataflow
---

import { FileTree } from "@astrojs/starlight/components";
import { Aside } from "@astrojs/starlight/components";

In this tutorial, we'll create a dataflow that takes a table containing two columns of time series data, and averages them along the time axis.

## Creating resources

To use tabular data in a dataflow, we need to define a **resource** to contain it.

First, reset your dataflow to remove any previous runs and return it to a clean state.
...
Your dataflow should now look like:
...
By convention, we put any input data files under a directory named `data` at the root of the dataflow. Create the `data` directory, open `data/tabulardata.csv` and paste the following table:
...
Next, we'll learn about working with multiple runs in a dataflow.
