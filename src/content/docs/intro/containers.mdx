...
By default, algorithms in opendata.studio run in the python-run-base Docker
...
create a custom Docker container to run your algorithm.
...
FROM opends/python-run-base:latest
...
This tells opendata.studio to use `mycontainer` when executing the `helloworld`
algorithm.
